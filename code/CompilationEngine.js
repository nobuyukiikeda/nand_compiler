...
const segment = kind => {
  if (kind === SymbolTableKinds.STATIC) {
    return Segments.STATIC;
  } else if (kind === SymbolTableKinds.FIELD) {
    return Segments.THIS;
  } else if (kind === SymbolTableKinds.VAR) {
    return Segments.LOCAL;
  } else if (kind === SymbolTableKinds.ARG) {
    return Segments.ARG;
  }
};
  ...
  *labelGenerator() {
  ...
  log({ type, data } = {}) {
    ...
    if (type === 'identifierToken') {
      ...
      str = `<${thisTokenType.display}> ${thisToken.display || thisToken} </${
        thisTokenType.display
      }>`;
    } else if (type === 'raw') {
      str = data;
    }
  ...
  tokenOneOf(accepted) {
    ...
    return (
      (Array.isArray(accepted) && accepted.includes(thisToken)) ||
  ...
  eat(accepted) {
    let ate = {
      token: this.getToken(this.tk.tokenType()),
      tokenType: this.tk.tokenType(),
    };
  ...
  compileClassVarDec() {
    ...
    let kind;
    if (type === STATIC) {
      kind = SymbolTableKinds.STATIC;
    } else if (type === FIELD) {
      kind = SymbolTableKinds.FIELD;
    }
    ...
    while (this.tokenOneOf(',')) {
      ...
      this.st.define(
        identifier,
        typeIdentifier.display || typeIdentifier,
        kind
      );
      this.log({
        type: 'identifierToken',
  ...
  compileStatements() {
    while (this.tokenOneOf([LET, IF, WHILE, DO, RETURN])) {
      const capitalized =
        this.tk.keyword().display[0].toUpperCase() +
        this.tk.keyword().display.slice(1);
      this.logWrapper(
        this[`compile${capitalized}Statement`],
        `${this.tk.keyword().display}Statement`
      );
    }
  }
  ...
  compileLetStatement() {
    ...
    if (this.tokenOneOf('[')) {
      ...
      this.vw.writePush(
        segment(this.st.kindOf(identifier)),
        this.st.indexOf(identifier)
      );
  ...
  compileSubroutineCall(identifier) {
    ...
    if (this.tk.symbol() === '.') {
      this.log({
        ...
        data: {
          ...logData,
          category: this.st.exists(logData.identifier)
            ? 'varName'
            : 'className',
        },
      });
      ...
      if (this.st.exists(identifier)) {
        // calling a method on an object identifier
      ...
      // calling method from the class that declares it
      this.log({
        type: 'identifierToken',
        data: { ...logData, category: 'subroutineName' },
      });
      this.eat('(');

      if (this.st.exists('this')) {
  ...
  compileTerm() {
    if (this.tokenOneOf(IDENTIFIER)) {
      ...
      const { token, tokenType } = this.eat([
        INT_CONST,
        STRING_CONST,
        ...KEYWORD_CONSTANT,
      ]);
